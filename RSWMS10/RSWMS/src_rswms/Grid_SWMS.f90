...
program grid
  IMPLICIT NONE
  Integer, Parameter :: ap=Kind(4)
  Integer(ap), Parameter :: dp=selected_real_kind(8)

  INTEGER(ap) :: i,j,k,l,m,n,o,p,ii,jj,geom,width,ICtype,MatNum,iCord
  INTEGER(ap) nex,ney,nez,nx,ny,nz,ax,by,np,nl,ne,ie,ix,iy,iz,iRest,layers,Xgradient
  !**** generates 3D-element mesh ******
  LOGICAL continu, bilayered
  REAL(dp) radius,zlim,htop,conctr,r,hinit,v1,v2,v3,v4,v5,x1min,x1max,x2min,x2max,z1min,z1max,z2min,z2max,y1min,y1max
...
21 DO ii=1,3
  ...
  CASE(3)
     READ (10,*) radius 
  CASE(4)
     READ (10,*) x1min, x1max, x2min, x2max, z1min, z1max, z2min, z2max
  CASE(5)
     READ (10,*) x1min, x1max
  CASE(6)
     READ (10,*) y1min, y1max 
...
120 ix=ix+1
  ...
  ELSE !(ICtype=3)
     OPEN (Unit=15,FILE='../in/outfem.new',STATUS='OLD',ERR=12)
	IF (i.EQ.1) THEN
     	READ (15,*)
     	READ (15,*)
     	READ (15,*)
  ...
  IF (abs(x).LT.1.E-7) x = 0._dp
  IF (abs(y).LT.1.E-7) y = 0._dp
  !************************************************************************
  ...
  IF (bilayered) THEN
     ...
     CASE(4) !rhizotron
        IF((x.LE.x1max) .AND. (x.GE.x1min)) MatNum=2
        IF((x.LE.x2max) .AND. (x.GE.x2min)) MatNum=2
        IF((z.LE.z1max) .AND. (z.GE.z1min)) MatNum=2
        IF((z.LE.z2max) .AND. (z.GE.z2min)) MatNum=2
     CASE(5) ! verticaly under x
        IF((x.LE.x1max) .AND. (x .GE. x1min)) MatNum=2
     CASE(6) ! verticaly under y
        IF((y.LE.y1max) .AND. (y .GE. y1min)) MatNum=2
     END SELECT
  ENDIF
